...
ServerBlock::ServerBlock(string listen, vector<string> server_names, string error_page, string client_max_body_size, string root, string index, vector<Location> locations)
{
	_listen = listen;
...
vector<string> ServerBlock::get_server_names() const { return _server_name; }
...
void ServerBlock::set_server_names(vector<string> server_name) { _server_name = server_name; }
